...
def getElement(code):
    ...
    try:
        if code.startswith("<html"):
            res = lxml.etree.parse(StringIO(code), parser).getroot()
        elif code.startswith("<body") or code.startswith("<head"):
            res = lxml.etree.parse(StringIO(code), parser).getroot()[0]
        else:
            res = lxml.etree.parse(StringIO(code), parser).getroot()[0][0]
        return res
    except:
        print(code)
        print(lxml.etree.parse(StringIO(code), parser).getroot())
        return lxml.etree.parse(StringIO(code), parser).getroot()
        #return None
...
def generate_random():
    return "onthefly_" + ''.join([random.choice(string.ascii_letters + string.digits) for n in range(11)]) + "_css"
...
def main(argv):
    ...
    if extension_folder.endswith("/"):
        extension = extension_folder.split("/")[-2]
    else:
        extension = extension_folder.split("/")[-1]

    XHOUND_FILTER = set()

    with open("detectable_doms_for5448.txt", "r") as fin:
        for l in fin.readlines():
            tmp = l.strip().split("\t")
            if tmp[0] != extension: continue
            for t in tmp[1:]:
                if t.startswith("NEW NODE:") or t.startswith("NEW FLYNODE:"):
                    k = urllib.parse.unquote_plus(t)
                    k = k.split("NODE: ")[1]
                    node = getElement(k)
                    for element in node.iter():
                        if not element.attrib.get("class", None) is None:
                            for v in element.attrib["class"].split(" "):
                                XHOUND_FILTER.add("." + v)
                        if not element.attrib.get("id", None) is None:
                            XHOUND_FILTER.add("#" + element.attrib["id"])
                elif t.startswith("ATTRIBUTE:"):
                    # TODO: extracting from modified or triggering content?!
                    k = urllib.parse.unquote_plus(t)
                    k = k.split("ATTRIBUTE: ")[1]
                    node = getElement(k)
                    for a in node.attrib.keys():
                        val = node.attrib[a]
                        if a == "id":
                            if val in ["header", "sampleform", "footer", "nav"]: continue
                            XHOUND_FILTER.add("#" + val)
                        elif a == "class":
                            for v in val.split(" "):
                                if v in ["twitter-share-button"]: continue
                                XHOUND_FILTER.add("." + v)
                    # Disabling triggering content...
    ...
    if not only_logging:
        ...
        for css_file in css_files:
            ...
            fout_css.write(sheet.cssText.decode("utf-8"))
            fout_css.close()
