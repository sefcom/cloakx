...
(function( window ) {
	...
	Sizzle.toSelector = function( tokens ) {
		var i = 0,
			len = tokens.length,
			selector = "";
		for ( ; i < len; i++ ) {
...
function maskCSSQuery(selector) {
    try {
    	var tokens = SizzleCSSTokenizer.tokenize(selector);
		for (var i in tokens) {
			var token = tokens[i];
			if (token.type == "CLASS") {
        ...
        return SizzleCSSTokenizer.toSelector(tokens);
    }
    catch(err) {
        console.log(err);
        return null;
...
function overrideInjections() {
    ...
    var prepareFinalContainer = function(container) {
        ...
        for (var i = 0; i < items_with_class.length; ++i) {
            ...
            var random_classes = node.prev_getAttribute("class");
            var final_class = "";
            for (var j in random_classes) {
            ...
            node["onthefly_real_className"] = final_class.trim();
        }
    }
